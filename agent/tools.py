...
try:
    ...
    # Initialize vision expert clients
...
def segment_and_mark(image, granularity:float = 1.8, alpha:float = 0.1, anno_mode:list = ['Mask', 'Mark']):
    ...
    """
    print("[VSP_TOOL_USED] segment_and_mark")
    
    if not VISION_TOOLS_AVAILABLE or som_client is None:
        raise RuntimeError("Vision tools are not available. This function requires vision expert services.")
...
def detection(image, objects, box_threshold:float = 0.35, text_threshold:float = 0.25):
    ...
    """
    print("[VSP_TOOL_USED] detection")
    
    if not VISION_TOOLS_AVAILABLE or gd_client is None:
        raise RuntimeError("Vision tools are not available. This function requires vision expert services.")
...
def depth(image):
    ...
    """
    print("[VSP_TOOL_USED] depth")
    
    if not VISION_TOOLS_AVAILABLE or da_client is None:
        raise RuntimeError("Vision tools are not available. This function requires vision expert services.")
...
def find_perpendicular_intersection(A, B, C):
    ...
    This function takes a coordinate A, start and end points of a line BC,
    and returns the coordinates of the point E on BC such that AE is perpendicular to BC.
    
    Args:
        A (List[float]): Coordinates of point A [x, y]
        B (List[float]): Start point of line BC [x, y]
        C (List[float]): End point of line BC [x, y]
    
    Returns:
        List[float]: Coordinates of point E [x, y]
    
    Example:
        A = [0, 0]
        B = [0, 4]
        C = [4, 0]
        E = find_perpendicular_intersection(A, B, C)
        print(E)  # Coordinates of the perpendicular intersection point
    """
    print("[VSP_TOOL_USED] find_perpendicular_intersection")
...
def find_parallel_intersection(A, B, C):
    """Calculate point E where AE is parallel to BC.
    
    This function takes a coordinate A, start and end points of a line BC,
    and returns the coordinates of the point E such that AE is parallel to BC.
    ...
    Example:
        ...
        B = [2, 0]
        C = [4, 2]
        E = find_parallel_intersection(A, B, C)
        print(E)  # Coordinates where AE is parallel to BC
    """
    print("[VSP_TOOL_USED] find_parallel_intersection")
    ...
    # For visualization, extend from A by the length of BC
    length_BC = np.linalg.norm(direction_BC)
    
    # Normalize the direction vector
    direction_AE_normalized = direction_BC / np.linalg.norm(direction_BC)
    
    # Point E can be found by moving from A in the direction of BC by the length of BC
